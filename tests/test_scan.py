...
def test_bordered_partial(bordered, fast, resize):
    em = bordered(resize=resize)
...
def test_bordered_full(bordered, fast, resize):
    em = bordered(resize=resize['resize'])
...
@pytest.mark.parametrize('fast', (True, False))
@pytest.mark.parametrize('resize', (None, 300))
def test_clear(clear, fast, resize):
    em = clear(resize=resize)
    em.scan(rand=0.1, fast=fast)
